...
function blueprint(){
  ...
  timeline
    ...
    .add('.hangar-copy', { opacity: [1, 0], y: [0, -14], duration: 520 }, 0)
    .add('.side-index', { opacity: [1, 0], x: [0, 18], duration: 480 }, 40)
    .add('.drone-hit, .computer-hit, .blueprint-hit, .scene-note', { opacity: [1, 0], scale: [1, 0.96], duration: 420 }, 120)
    .call(() => transition.classList.add('active'), 900)
    .call(() => { app.classList.add('blueprint-on'); window.scrollTo({ top: 0, behavior: 'instant' }); }, 1180)
    .call(() => { transition.classList.remove('active'); app.classList.remove('uav-inspecting'); }, 1900);
}
...
document.querySelector('#returnHangar').addEventListener('click', () => { app.classList.remove('blueprint-on'); close(detail); window.dispatchEvent(new Event('uav:return')); animate('.hangar-copy', { opacity: [0, 1], y: [-14, 0], duration: 560, ease: 'out(4)' }); animate('.side-index', { opacity: [0, 1], x: [18, 0], duration: 580, ease: 'out(4)' }); animate('.drone-hit, .computer-hit, .blueprint-hit, .scene-note', { opacity: [0, 1], scale: [0.96, 1], duration: 460, ease: 'out(4)', delay: 120 }); document.querySelector('#hangar').scrollIntoView({behavior:'smooth'}); });
