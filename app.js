...
function blueprint(){
  ...
  app.classList.add('uav-inspecting');
  window.dispatchEvent(new Event('uav:inspect'));
  window.setTimeout(() => transition.classList.add('active'), 900);
  window.setTimeout(() => { app.classList.add('blueprint-on'); window.scrollTo({top:0, behavior:'instant'}); }, 1170);
  window.setTimeout(() => { transition.classList.remove('active'); app.classList.remove('uav-inspecting'); }, 1860);
}
...
document.querySelector('#returnHangar').addEventListener('click', () => { app.classList.remove('blueprint-on'); close(detail); window.dispatchEvent(new Event('uav:return')); document.querySelector('#hangar').scrollIntoView({behavior:'smooth'}); });
